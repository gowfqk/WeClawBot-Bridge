...
export type WsServerInbound = WsAuthMessage | WsChatReply | WsPushMessage | { type: 'pong' }
export type WsServerOutbound = WsChatMessage | WsErrorMessage | { type: 'auth_ok'; agentId: string } | { type: 'auth_fail'; reason: string } | { type: 'ping' }
...
export class WsAgentServer {
  ...
  async invoke(agentId: string, payload: AgentPayload, timeout = 60000): Promise<AgentResponse> {
