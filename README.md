...
添加后点击「生成 Token」，复制安装命令到对应 Gateway 端即可接入。Token 保存后可随时通过列表「Token」按钮查看。

### 7. 接入 Hermes / OpenClaw Gateway

Bridge 的 WS Remote Agent 协议可直连 Hermes 或 OpenClaw Gateway，无需中间 SDK：

| 后端 | 仓库 | 安装方式 |
|---|---|---|
| Hermes | [hermes-weclawbot-channel](https://github.com/gowfqk/hermes-weclawbot-channel) | `cp` 到 `~/.hermes/plugins/` + `hermes plugins enable` |
| OpenClaw | [openclaw-weclawbot-channel](https://github.com/gowfqk/openclaw-weclawbot-channel) | `openclaw plugins install --link` |
| QwenPaw | [qwenpaw-weclawbot-channel](https://github.com/gowfqk/qwenpaw-weclawbot-channel) | `qwenpaw plugin install` |

三个后端可**并行运行**，微信内通过 `#<命令>` 切换：
